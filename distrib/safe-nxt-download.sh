...
wget -nv https://bitbucket.org/JeanLucPicard/nxt/downloads/nxt-client-$1.{zip,jar,zip.asc,jar.asc,changelog.txt.asc} > /dev/null 2>&1
zip_sig=$(gpg --verify nxt-client-$1.zip.asc > /dev/null 2>&1; echo $?)
jar_sig=$(gpg --verify nxt-client-$1.jar.asc > /dev/null 2>&1; echo $?)
...
if [[ sha_sum -ne 0 || jar_sig -ne 0 || zip_sig -ne 0 || chg_sig -ne 0 ]]; then
	rm -f nxt-client-$1.*
